...
class MapGen(object):
    ...
    def __init__(self):
        self.latitude = 45.378906
        self.longitude = -74.106506    
        self.zoom = 14            
        self.scale=1
    ...
    def generateImage(self):            
        ...
        pixel_x, pixel_y = self.LatLonToPixels(self.latitude, self.longitude,self.zoom)
        for x in range(0, self.map_tile_width):
            for y in range(0, self.map_tile_height) :
                ...
                final_lat,final_long=self.PixelsToLat(pixel_x+self.size_tile_x*x,pixel_y+((self.size_tile_y-50)*y),self.zoom)
